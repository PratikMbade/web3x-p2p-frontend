...
var S = (e, a) => {
  for (var t in a) V(e, t, { get: a[t], enumerable: true });
};
var P = {};
S(P, { authenticate: () => v, isAutocompleteAvailable: () => E, isAvailable: () => x, isLocalAuthenticator: () => L, register: () => k });
var s = {};
S(s, { bufferToHex: () => O, concatenateBuffers: () => C, isBase64url: () => A, parseBase64url: () => o, parseBuffer: () => K, sha256: () => l, toBase64url: () => c, toBuffer: () => b });
function b(e) {
...
async function k(e) {
  ...
  let t = { challenge: o(e.challenge), rp: { id: e.domain ?? window.location.hostname, name: e.domain ?? window.location.hostname }, user: { id: b(a.id), name: a.name, displayName: a.displayName ?? a.name }, hints: e.hints, pubKeyCredParams: [{ alg: -7, type: "public-key" }, { alg: -257, type: "public-key" }], timeout: e.timeout, authenticatorSelection: { userVerification: e.userVerification, authenticatorAttachment: H(e.hints), residentKey: e.discoverable ?? "preferred", requireResidentKey: e.discoverable === "required" }, attestation: e.attestation ?? true ? "direct" : "none", ...e.customProperties };
  console.debug(t), f == null ? void 0 : f.abort("Cancel ongoing authentication"), f = new AbortController();
  let i = await navigator.credentials.create({ publicKey: t, signal: f == null ? void 0 : f.signal }), r = i.response;
...
async function v(e) {
  ...
  let t = await navigator.credentials.get({ publicKey: a, mediation: e.autocomplete ? "conditional" : void 0, signal: f == null ? void 0 : f.signal });
  if (t.type != "public-key") throw "Unexpected credential type!";
  f = null, console.debug(t);
  let i = t.response;
...
var w = {};
S(w, { parseCryptoKey: () => R, randomChallenge: () => z, verifyAuthentication: () => _, verifyRegistration: () => Z, verifySignature: () => U });
var h = {};
S(h, { getAlgoName: () => B, parseAuthentication: () => j, parseAuthenticator: () => u, parseClient: () => I, parseRegistration: () => X, toAuthenticationInfo: () => g, toRegistrationInfo: () => N });
...
async function Z(e, a) {
  let t = I(e.response.clientDataJSON), i = u(e.response.authenticatorData), r = i.aaguid;
  if (a.verbose && (console.debug(t), console.debug(i)), a.userVerified && !i.flags.userVerified) throw new Error("User verification required but not satisfied.");
  let n = a.domain ?? new URL(t.origin).hostname, d = c(await l(b(n)));
  if (i.rpIdHash !== d) throw new Error(`Unexpected RpIdHash: ${i.rpIdHash} vs ${d}`);
  if (!r) throw new Error("Unexpected error, no AAGUID.");
  ...
  return h.toRegistrationInfo(e, i);
}
async function _(e, a, t) {
  ...
  let r = I(e.response.clientDataJSON), n = u(e.response.authenticatorData);
  ...
  let d = t.domain ?? new URL(r.origin).hostname, y = c(await l(b(d)));
  if (n.rpIdHash !== y) throw new Error(`Unexpected RpIdHash: ${n.rpIdHash} vs ${y}`);
...
async function U({ algorithm: e, publicKey: a, authenticatorData: t, clientData: i, signature: r, verbose: n }) {
  let d = await R(e, a);
  n && console.debug(d);
  let y = await l(o(i)), m = C(o(t), y);
  n && (console.debug("Algorithm: " + e), console.debug("Public key: " + a), console.debug("Data: " + c(m)), console.debug("Signature: " + r));
  let D = o(r);
  e == "ES256" && (D = q(D));
  let Y = T(e);
  return await crypto.subtle.verify(Y, d, D, m);
}
function q(e) {
  let a = new Uint8Array(e), t = a[4] === 0 ? 5 : 4, i = t + 32, r = a[i + 2] === 0 ? i + 3 : i + 2, n = a.slice(t, i), d = a.slice(r);
  return new Uint8Array([...n, ...d]);
}
...
var PasskeyWebClient = class {
  ...
  async register(args) {
    ...
    const registration = await P.register({
      attestation: true,
      challenge,
      domain: rp.id,
      user: name,
    ...
    return {
      ...
      credential: {
        algorithm: h.getAlgoName(registration.response.publicKeyAlgorithm),
...
//# sourceMappingURL=chunk-JXDMZFUH.js.map
